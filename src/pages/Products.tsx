...
const Products: React.FC<ProductsProps> = ({}) => {
  ...
  const columns: GridColDef[] = [
    { field: "id", headerName: "id", width: 200, renderCell: (params: GridCellParams) => (
        <Typography variant="body2">{params.value?.toString().substr(0, 10)}</Typography>
    ) },
  ...
  const handleUpdate = () => {
    const exists = categories?.findIndex(
      (item: categoryType) => item.title === selected.categoryName
    );
    if (exists !== -1) {
      console.log(previewSource);
      setLoading(true);
      db.collection("product_categories")
        .doc(`${selected.categoryName}`)
        .get()
        .then((doc) => {
          if (doc && doc.exists) {
            const d = doc?.data();
                      ...
                      .then((url) => {
                        ...
                        db.collection("product_categories")
                          ...
                          .then(() => {
                            ...
                            storage
                              .ref()
                              .child(`category_images/${selected.categoryName}`)
                              .delete()
                              .then(() => {
                                console.log("deleted");
                              })
                        ...
                        db.collection("product_categories")
                          ...
                          .then(() => {
                            ...
                            if (name !== d?.imageName) {
                              storage
                                .ref()
                                .child(`category_images/${d?.imageName}`)
                                .delete()
                                .then(() => {
                                  console.log("deleted");
                                })
  ...
  const handleCreate = () => {
    const exists = categories?.findIndex(
      (item: categoryType) => item.title === name
    );
    console.log(exists);
    if (exists === -1) {
      const metadata = {
  ...
  return (
    <div className={classes.container}>
        ...
        <div className={classes.modalContainer}>
          {loading && (
            <div className={classes.loaderContainer}>
              <CircularProgress
                size={28}
                style={{ color: colors.deepPrimary }}
              />
            </div>
          )}
          ...
          <Typography
            variant="h6"
            style={{ color: "#ccc", marginLeft: 12, fontWeight: 200 }}
          >
            New Product
          </Typography>
          <div className={classes.formContainer}>
            <div className={classes.inputsContainer}>
              <div className={classes.inputContainer}>
                ...
                {selected ? (
                  <Button
                    variant="outlined"
                    className={classes.createModalBtn}
            ...
            <Paper
              elevation={0}
              className={clsx(classes.logoContainer, classes.boxShadow)}
            >
              <div
                onClick={() => {
              ...
              {!selected ? (
                <Avatar
                  ...
                  alt="Vendor Logo"
                  src={previewSource ? previewSource.toString() : undefined}
                />
              ) : (
                <Avatar
                  className={classes.avatar}
                  alt="Vendor Logo"
                  src={previewSource ? previewSource.toString() : selected.icon}
                />
              )}
            </Paper>
          </div>
      ...
      <div style={{ display: "flex" }}>
        <Button
          variant="outlined"
          className={`${classes.createBtn} box-shadow`}
          onClick={() => {
            setName("");
            setOpen(true);
          }}
